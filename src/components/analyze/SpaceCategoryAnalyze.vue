...
import { getCategoryAnalyzeUsingPost } from '@/api/kongjianfenximokuai.ts'
...
const fetchData = async () => {
  loading.value = true
  const res = await getCategoryAnalyzeUsingPost({
    queryAll: props.queryAll,
    queryPublic: props.queryPublic,
    spaceId: props.spaceId,
  })
  if (res.code === 0 && res.data) {
    dataList.value = (res.data as API.SpaceCategoryAnalyzeVo[]) ?? []
  } else {
